...
def get_data_from_kaldi(files, spk2utt, vadDict, FLAG):
    print 'get_data_from_kaldi [{:s}] FLAG: [{:s}]'.format(files, str(FLAG))
    ...
    for spkID, spkName in enumerate(spk2utt):
        ...
        for id_, uttName in enumerate(spk2utt[spkName]):
            ...
            sys.stdout.flush()
    print ''
    print 'Finished get_data_from_kaldi Load\n'
    return dataDict

def get_data_from_logfbank(feaPath, spk2utt, vadDict, FLAG):
    check_path(feaPath)
    print 'get_data_from_logfbank [{:s}] FLAG: [{:s}]'.format(feaPath, str(FLAG))
    dataDict = {}
    stime = time.time()
    for spkID, spkName in enumerate(spk2utt):
        dataDict[spkID] = []
...
def process(feaType, basePath, feaPath, savePath, mode, FLAG):
    print '----------------------------------------------------------------'
    print 'feaType: [{:s}] mode: [{:s}] FLAG: [{:s}]'.format(feaType, mode, str(FLAG))
    check_path(basePath)
    check_path(feaPath)
    ...
    dataDict = None
    if feaType == 'kaldi':
        dataDict = get_data_from_kaldi(os.path.join(basePath, 'feats.scp'), spk2utt, vadDict, FLAG)
    elif feaType == 'speech':
        dataDict = get_data_from_logfbank(feaPath, spk2utt, vadDict, FLAG)
    else:
        raise IOError('cannot found feaType[{:s}]'.format(feaType))
    pickle.dump(dataDict, open(os.path.join(dataPath, mode+'.dict'), 'wb'),
                protocol=pickle.HIGHEST_PROTOCOL)
  
    print 'dataDict save in [{:s}]'.format(os.path.join(dataPath, mode+'.dict')) 
...
if __name__ == '__main__':
    ...
    process('speech', enroFarPath, feaPath, savePath, 'enro_far_1214', FLAG)
    process('speech', testFarPath, feaPath, savePath, 'test_far_1214', FLAG)
